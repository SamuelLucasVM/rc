    ...
    def send_updates_to_neighbors(self):
        ...
        for neighbor_address in self.neighbors:
            ...
            for net, info in self.routing_table.items():
                if getattr(self, 'use_split_horizon', False) and info['next_hop'] == neighbor_address:
                    tabela_customizada[net] = {'cost': 16, 'next_hop': self.my_address}
                else:
                    tabela_customizada[net] = info
                    
            tabela_para_enviar = summarize_routes(tabela_customizada)
            ...
            try:
                ...
                for network, info in list(self.routing_table.items()):
                    if info['next_hop'] == neighbor_address and info['cost'] < 16:
                        self.routing_table[network]['cost'] = 16
                        mudou = True
                
                if mudou:
                    print("Tabela após registrar a queda do vizinho:")
                    print(json.dumps(self.routing_table, indent=4))
# --- API Endpoints ---
...
if __name__ == '__main__':
    ...
    parser.add_argument('--interval', type=int, default=12, help="Intervalo de atualização periódica em segundos.")
    parser.add_argument('--protection', action='store_true', help="Habilita a proteção Split Horizon / Poison Reverse")
    ...
    ip_do_container = socket.gethostbyname(hostname)
    my_full_address = f"{ip_do_container}:{args.port}"
    ...
    router_instance.use_split_horizon = args.protection
